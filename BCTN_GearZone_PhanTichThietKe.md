...
flowchart LR
    ...
    subgraph SYS["Hệ thống GearZone"]
        subgraph G_AUTH["Xác thực"]
            UC_DK("Đăng ký")
            UC_DN("Đăng nhập")
            UC_DX("Đăng xuất")
        end

        subgraph G_ORDER["Đặt hàng"]
            UC_DH("Đặt hàng")
            UC_HUY("Hủy đặt hàng")
            UC_TD("Theo dõi đơn hàng")
        end

        subgraph G_INT["Tương tác"]
            UC_DG("Viết đánh giá")
        end

        subgraph G_PAY["Thanh toán"]
            UC_TT("Thanh toán")
            UC_COD("Thanh toán COD")
            UC_OL("Thanh toán online")
            UC_XN("Xác nhận thanh toán online")
        end

        subgraph G_ADMIN["Quản trị"]
            UC_QLSP("Quản lý sản phẩm")
            UC_QLDH("Quản lý đơn hàng")
            UC_QLDM("Quản lý danh mục")
            UC_QLND("Quản lý người dùng")
            UC_CFG("Cấu hình hệ thống")
        end
...
Hình 2-2: Sơ đồ use case tổng quát hệ thống GearZone.

> **Ghi chú vị trí actor:** ADMIN và NHÂN VIÊN KHO đặt bên **phải** System Boundary (đối xứng với KHÁCH HÀNG bên trái). HỆ THỐNG THANH TOÁN đặt ngoài cùng bên phải kết nối vào nhóm Thanh toán.
