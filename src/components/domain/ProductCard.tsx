...
export function ProductCard({ product, accent, showBadge = false, badgeText }: ProductCardProps) {
  ...
  return (
    <div
      className="group relative p-1.5 rounded-[1.25rem] bg-white/[0.03] ring-1 ring-white/[0.06] transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] hover:-translate-y-2 hover:ring-white/[0.12]"
    >
      ...
      >
        ...
        <div className="p-5 flex flex-col flex-grow">
          ...
          <div className="mt-4 flex items-center justify-between text-[11px] font-semibold text-slate-500 border-t border-white/[0.04] pt-3">
            <span className={product.stock > 0 ? 'text-emerald-500/90' : 'text-rose-500/90'}>
          ...
          <div className="mt-auto pt-4 border-t border-white/[0.04] flex items-center justify-between gap-2">
            ...
            <button 
              ...
              className="h-9 px-2.5 md:px-3 rounded-full bg-gradient-to-r from-emerald-600 to-cyan-600 hover:from-emerald-500 hover:to-cyan-500 text-white disabled:cursor-not-allowed disabled:bg-slate-800 disabled:text-white/30 active:scale-[0.97] transition-all duration-300 ease-out-expo flex items-center justify-center gap-1.5"
              title="Thêm vào giỏ hàng"
            >
              <ShoppingCart className="size-3.5" />
              <span className="text-[11px] font-extrabold">Mua ngay</span>
            </button>
          </div>
        </div>
      </article>
