            ...
            <div className="bg-slate-900 border border-white/5 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              ...
              <div className="flex gap-2 shrink-0">
                ...
                {!['CANCELLED', 'REFUNDED'].includes(order.status) && (
                  <button type="button" onClick={() => onConfirmAction({ type: 'CANCEL', orderId: order.id })} className="px-3 py-1.5 bg-slate-800 hover:bg-red-500/20 hover:text-red-400 text-slate-300 rounded-lg text-xs font-bold transition-colors">
                    Hủy đơn
                  </button>
                )}
              </div>
